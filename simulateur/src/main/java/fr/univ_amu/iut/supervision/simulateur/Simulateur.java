...
public final class Simulateur {
    ...
    public static void main(String[] args) {
        int nbBoitiers = args.length > 0 ? Integer.parseInt(args[0]) : 24;
        String hote = env("MQTT_HOST", "localhost");
        ...
        for (int i = 1; i <= nbBoitiers; i++) {
            ...
            byte[] octets = TrameCodec.encoder(new Trame(1));
            String hex = Hex.depuisOctets(octets);
            client.publishWith()
                    .topic("simulateur/" + id)
