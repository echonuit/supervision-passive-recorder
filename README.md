...
le télécharge à la première exécution et installe les hooks git du dépôt.
    ...
    ./mvnw verify                       # construit, formate, teste, analyse (PMD) et vérifie la couverture
    ./mvnw pmd:pmd                      # rapport PMD lisible dans target/site/pmd.html
    ...
    cd simulateur && ../mvnw exec:exec  # publie des trames sur le courtier MQTT
                                        # -Dsimulateur.boitiers=3 pour en simuler moins
...
4. `docs/decisions/` — pourquoi chaque outil est là. Commencer par
   `0002-javalin.md` : pourquoi cette pile plutôt que Spring Boot.
...
`./mvnw verify` — la même commande que la CI — enchaîne quatre gardes :

| Garde | Ce qu'il vérifie | Bloquant |
|---|---|---|
| Spotless | Le formatage | Oui, mais le hook pre-commit le corrige avant |
| Tests | JUnit, AssertJ, Testcontainers | Oui |
| PMD | Garde-fous de conception, voir `pmd-ruleset.xml` | Oui |
| JaCoCo | Plancher de couverture : 80 % de lignes, 50 % de branches | Oui |

Deux principes valent la peine d'être connus avant de les rencontrer :

- **Une violation PMD se corrige, elle ne se tait pas.** Ni `@SuppressWarnings("PMD...")`,
  ni `//NOPMD`. Si un seuil est vraiment inadapté, on rediscute le seuil dans
  `pmd-ruleset.xml`, avec l'exemple sous les yeux.
- **Le plancher de couverture est un cliquet, pas une cible.** Il empêche de
  régresser. On le relève quand la couverture monte, on ne le baisse pas quand
  elle descend.

En cas de CI rouge, les rapports PMD et JaCoCo sont déposés en artefact du job.
